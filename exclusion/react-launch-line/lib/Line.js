...
export default class Bar extends React.Component {
  ...
  componentWillUpdate(nextProps, nextState) {
    if (nextProps.value !== this.props.value) {
      ...
      var parsedNext = parseFloat(nextProps.value); // console.log("Will Change Value!");
  ...
  goTween(prop, start, end, duration = 500, direction = 1, easing = 'Linear') {
    ...
    return new Promise((resolve, reject) => {
      ...
      var time = timer((elapsed, d) => {
        ...
        let value = i(progress); // num = value;
        ...
        this._updateStateValue(prop, value); // _self.setState({width: value})
